...
class BaseHeadSplit(nn.Module):
    def __init__(self, base, head):
        ...
        self.base = base
        self.head = head
        self.classifier_weight_keys = ['head.weight', 'head.bias']
        self.base_weight_keys = self.get_param_names(base)
    ...
    def get_param_names(module):
        """Helper function to get the names of all parameters in a module."""
        param_names = []
        for name, param in module.named_parameters():
            param_names.append('base.' + name)
        return param_names


class BasePerHeadSplit(nn.Module):
    def __init__(self, base, per_dim, num_classes):
        super(BasePerHeadSplit, self).__init__()
        self.base = base
        base_out_dim = self._get_base_out_dim(base)
        device = next(base.parameters()).device
        self.per = nn.Sequential(
            nn.Linear(base_out_dim, base_out_dim),
            nn.ReLU(),
            nn.Linear(base_out_dim, base_out_dim)
        ).to(device)
        self.head = nn.Linear(base_out_dim, num_classes).to(device)
        self.classifier_weight_keys = ['head.weight', 'head.bias']
        self.base_weight_keys = self.get_param_names(base)
        self.per_weight_keys = ['per.0.weight', 'per.0.bias', 'per.2.weight', 'per.2.bias']

    def _get_base_out_dim(self, base):
        with torch.no_grad():
            device = next(base.parameters()).device
            for shape in self._get_input_shape_candidates(base):
                try:
                    ...
                    continue
        raise ValueError("Cannot determine base output dimension")

    def _get_input_shape_candidates(self, base):
        for module in base.modules():
            if isinstance(module, nn.Conv2d):
                c = module.in_channels
                return [(c, 28, 28), (c, 32, 32), (c, 64, 64), (c, 224, 224)]
            elif isinstance(module, nn.Linear):
                return [(module.in_features,)]
        return [(1, 28, 28), (3, 32, 32)]
