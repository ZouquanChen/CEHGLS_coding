...
class Server(object):
    def __init__(self, args, times):
        ...
        self.use_grad_compress = args.use_grad_compress
        self.use_head_grad = args.use_head_grad
    ...
    def send_models(self):
        ...
        for client in self.clients:
            ...
            if self.use_head_grad:
                # HGLS mode: only send base parameters, keep personalized head on client
                client.set_base_parameters(self.global_model)
            else:
    ...
    def save_global_model(self):
        ...
        flcore_dir = os.path.dirname(servers_dir)
        model_dir = os.path.join(flcore_dir, "models", self.dataset)

        if not os.path.exists(model_dir):
            os.makedirs(model_dir)
        model_path = os.path.join(model_dir, self.algorithm + "_server" + ".pt")
        ...
        try:
            ...
            print(f"[Warning] Direct save failed: {e}, trying temporary file...")
            fd, tmp_path = tempfile.mkstemp(suffix=".pt", dir=model_dir)
            os.close(fd)
            try:
                torch.save(self.global_model, tmp_path)
                if os.path.exists(model_path):
                    try:
                        os.remove(model_path)
                    except Exception:
                ...
                print(
                    f"[Warning] Temporary file also failed: {e2}, model saved to: {tmp_path}"
                )
    ...
    def save_results(self):
        ...
        current_dir = os.path.dirname(os.path.abspath(__file__))
        system_dir = os.path.dirname(os.path.dirname(os.path.dirname(current_dir)))
        result_path = os.path.join(system_dir, "results")
        ...
        if len(self.rs_test_acc):
                ...
                try:
                    ...
                    try:
                        ...
                        file_path = os.path.join(
                            result_path, "{}_{}.h5".format(algo, ts)
                        )
                        print(f"Original file occupied, saving to: {file_path}")
            ...
            try:
                ...
                fallback_path = os.path.join(fallback_dir, "{}.h5".format(algo))
                print(f"Permission error, saving to: {fallback_path}")
                with h5py.File(fallback_path, "w") as hf:
                    hf.create_dataset("rs_test_acc", data=self.rs_test_acc)
                    hf.create_dataset("rs_test_acc_std", data=self.rs_test_acc_std)
                    hf.create_dataset("rs_test_auc", data=self.rs_test_auc)
                    hf.create_dataset("rs_train_loss", data=self.rs_train_loss)
    ...
    def evaluate(self, acc=None, loss=None):
        ...
        print(
            "Test client acc:[{}]".format(",".join(["{:.4f}".format(a) for a in accs]))
        )
    ...
    def mm(model):
        ...
        for param in model.parameters():
            ...
            param_size_bytes = element_num * element_size_bytes
            total_size_bytes += param_size_bytes

        return total_size_bytes

    @staticmethod
    def memory_size(torch_obj):
        """
        :param torch_obj: Tensor to calculate
        Args:
            torch_obj:

        Returns: Tensor memory size

        """

        total_memory_size = sum(torch_obj.element_size() * torch_obj.numel())
        return total_memory_size

    @staticmethod
    def get_grad_memory(grads):
        """Calculate memory usage of gradient list (bytes)"""
        total = 0
        for grad in grads:
            for param in grad:
